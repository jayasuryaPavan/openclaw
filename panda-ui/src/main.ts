...
import { INITIAL_STATE, transition, type AppState } from "./state.js";
import { EMPTY_HISTORY, appendMessage, type HistoryState } from "./history.js";

// ── State ──────────────────────────────────────────────────────────────────
let appState: AppState = INITIAL_STATE;
let historyState: HistoryState = EMPTY_HISTORY;
let busy = false;
...
function renderHistory(history: HistoryState) {
  historyEl.innerHTML = "";
...
if (!isSttSupported()) {
  ...
  const stt = createStt({
    ...
    onError: (msg) => {
      ...
      if (msg !== "no-speech") showError(`Mic error: ${msg}`);
      update(transition(appState, "idle", "Say something…"));
    },
    onEnd: () => {
      sttActive = false;
